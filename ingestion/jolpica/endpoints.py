...
from ingestion.jolpica.client import paginate_all, raw
...
def get_seasons() -> list[JolpicaSeason]:
    ...
    return [JolpicaSeason(year=int(r["season"]), url=r["url"], _raw_json=raw(r)) for r in records]
    ...
    for r in records:
        ...
        out.append(
            JolpicaCircuit(
                ...
                length_km=float(r["length"]) if r.get("length") else None,
                corners=int(r["turns"]) if r.get("turns") else None,
                _raw_json=raw(r),
            )
        )
    return out
        ...
        for r in race.get("Results", []):
            out.append(
                JolpicaResult(
                    ...
                    position_order=int(r["position"])
                    if r.get("position", "").isdigit()
                    else int(r.get("positionOrder", 0)),
                    ...
                    fastest_lap_speed=float(r["FastestLap"]["AverageSpeed"]["speed"])
                    if r.get("FastestLap") and r["FastestLap"].get("AverageSpeed")
                    else None,
                    _raw_json=raw(r),
                )
            )
...
def get_driver_standings(season: int, round_num: int) -> list[JolpicaDriverStanding]:
    records = paginate_all(
        f"{season}/{round_num}/driverstandings", "StandingsTable", "StandingsLists"
    )
    out = []
    for sl in records:
        for r in sl.get("DriverStandings", []):
            out.append(
                JolpicaDriverStanding(
                    ...
                    round=round_num,
                    driver_id=r["Driver"]["driverId"],
                    constructor_id=r["Constructors"][0]["constructorId"]
                    if r.get("Constructors")
                    else "",
...
def get_constructor_standings(season: int, round_num: int) -> list[JolpicaConstructorStanding]:
    records = paginate_all(
        f"{season}/{round_num}/constructorstandings", "StandingsTable", "StandingsLists"
    )
    out = []
    for sl in records:
